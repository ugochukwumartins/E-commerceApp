...
import 'email_password_signIn_controller.dart';
...
class _EmailPasswordSignInContentsState
  ...
  Future<void> _submit(EmailPasswordSignInState state) async {
    ...
    if (_formKey.currentState!.validate()) {
      final controller =
          ref.read(emailPasswordSignInProvider(widget.formType).notifier);
  ...
  void _updateFormType(EmailPasswordSignInFormType formType) {
    ...
    ref
        .read(emailPasswordSignInProvider(widget.formType).notifier)
  ...
  Widget build(BuildContext context) {
    ref.listen<AsyncValue>(
        emailPasswordSignInProvider(widget.formType)
            .select((state) => state.value), (_, state) {
      state.showAlertDialogOnError(context);
    });
    final state = ref.watch(emailPasswordSignInProvider(widget.formType));
